    ...
    <Container w={['90%', '80%']} maxW="1200px" p="2rem 0">
      <Box display={{ lg:'block', xl:'grid'}} w="100%" alignItems="center" gridTemplateColumns="1fr 1fr" gridGap="3rem">
          <Box order={whichSide ? "1" : "2"} pos="relative">
            <Link _disabled={!link} cursor={!link ? "not-allowed" : "pointer"} href={link} w="100%" display="grid" onMouseOver={over} onMouseOut={out} mb={{base: '6', xl: '0'}}>
          ...
          <Box order={whichSide ? "2" : "1"} pos="relative">
            ...
            <Link _disabled={!link} href={link}><Button bg={!link ? "brand.grays.500" : "brand.gray"} color="brand.white" cursor={!link ? "not-allowed" : "pointer"} _hover={{ bg: "brand.grays.500" }}>{linkText ?? 'View Project'}</Button></Link>
          </Box>
      </Box>
    </Container>
  );
