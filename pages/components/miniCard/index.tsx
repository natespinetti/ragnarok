...
export const MiniCard = ({ fields }: TypeMiniCard) => { 
  ...
  return (
      <Box display="block" w="100%" alignItems="center">
          <Box pos="relative">
            <Link isDisabled={!link} cursor={!link ? "not-allowed" : "pointer"} href={link} w="100%" display="grid" onMouseOver={over} onMouseOut={out} mb={{base: '6', xl: '0'}}>
          ...
          <Box pos="relative" pt="1rem">
            ...
            <Link isDisabled={!link} cursor={!link ? "not-allowed" : "pointer"} href={link}><Button bg={!link ? "brand.grays.500" : "brand.gray"} cursor={!link ? "not-allowed" : "pointer"} color="brand.white" _hover={{ bg: "brand.grays.500" }}>{linkText ?? 'View Project'}</Button></Link>
          </Box>
      </Box>
  );
};
